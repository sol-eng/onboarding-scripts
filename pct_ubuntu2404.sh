...
################################################################################
# Posit Connect base install (Ubuntu 24.04)
...
# - Install Posit Connect (PINNED via CONNECT_VERSION)
...
install_connect() {
  ...
  # Validate CONNECT_VERSION format: YYYY.MM.PATCH
  if [[ ! "$CONNECT_VERSION" =~ ^[0-9]{4}\.[0-9]{2}\.[0-9]+$ ]]; then
    echo "ERROR: CONNECT_VERSION must be in YYYY.MM.PATCH format (e.g. 2025.12.1)"
    exit 1
  fi

  # Derive series from version (YYYY.MM)
  local connect_series
  connect_series="${CONNECT_VERSION%.*}"

  log "Installing Posit Connect ${CONNECT_VERSION} (series ${connect_series})..."

  local deb="rstudio-connect_${CONNECT_VERSION}~${CONNECT_UBUNTU_TAG}_${arch}.deb"
  local url="https://cdn.posit.co/connect/${connect_series}/${deb}"

  log "Downloading Connect from: ${url}"
  (cd "$WORKDIR" && curl -fLO "$url")

  log "Installing Connect..."
  install_local_deb "${WORKDIR}/${deb}"

  log "Enabling and starting rstudio-connect service..."
  systemctl daemon-reload
  systemctl enable --now rstudio-connect || true

  log "Connect status:"
  systemctl --no-pager --full status rstudio-connect || true
}
...
smoke_tests() {
  ...
  log "Connect service + listener (default port 3939):"
  systemctl is-active rstudio-connect || true
  ss -lntp | grep 3939 || true
